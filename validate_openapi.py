...
try:
    ...
    print(f'✓ Responses: {len(data.get("components", {}).get("responses", {}))} responses')
    print(f'✓ Security Schemes: {len(data.get("components", {}).get("securitySchemes", {}))} schemes')
    ...
    print(f'      even if not explicitly marked in the OpenAPI spec.')
    
    # List all endpoints
    print('\nEndpoints found:')
    for path in sorted(data.get('paths', {}).keys()):
        methods = [m.upper() for m in data['paths'][path].keys() if m in ['get', 'post', 'put', 'patch', 'delete']]
