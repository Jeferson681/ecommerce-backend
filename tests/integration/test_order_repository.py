...
def test_order_repository_crud_flow() -> None:
    ...
    fetched_by_user = order_repo.list_by_user(1)
    ...
    order_repo.create_item(item)
    session.commit()

    fetched_item = session.get(OrderItem, item.id)
    ...
    updated = session.get(OrderItem, item.id)
    ...
    deleted = session.get(OrderItem, item.id)
